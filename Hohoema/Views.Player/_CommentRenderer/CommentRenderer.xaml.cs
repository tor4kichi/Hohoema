...
namespace Hohoema.Views
{
    public sealed partial class CommentRenderer : UserControl
    {
                ...
                using (var releaser = await _UpdateLock.LockAsync())
                {
                    ...
                    if (PlaybackState == MediaPlaybackState.Playing)
                    {
                        ...
                        foreach (var renderComment in CommentCanvas.Children.Cast<CommentUI>())
                        {
                            ...
                            {
                                var comment = renderComment.DataContext as CommentViewModel;
                                if (_animationSetMap.TryGetValue(renderComment.DataContext as CommentViewModel, out var anim))
                                {
                ...
                me.CommentItemsChangedSubscriber = newNCC.CollectionChangedAsObservable()
                    .Subscribe(async args =>
                    {
                        await dispatcher.RunAsync(Windows.UI.Core.CoreDispatcherPriority.Normal, async () => 
                        {
                            ...
                            {
                                await me.AddCommentToCanvasAsyncSafe(args.NewItems.Cast<CommentViewModel>());
                            }
                            else if (args.Action == NotifyCollectionChangedAction.Remove)
                            {
                                await me.RemoveCommentFromCanvasAsyncSafe(args.OldItems.Cast<CommentViewModel>());
                            }
                        });
                    });
            }
        ...
        private async void Ov_VectorChanged(IObservableVector<object> sender, IVectorChangedEventArgs args)
        {
            ...
            {
                await AddCommentToCanvasAsyncSafe(sender[(int)args.Index] as CommentViewModel);
            }
            else if (args.CollectionChange == CollectionChange.ItemRemoved)
            {
                await RemoveCommentFromCanvasAsyncSafe(sender[(int)args.Index] as CommentViewModel);
            }
        ...
        Dictionary<CommentViewModel, AnimationSet> _animationSetMap = new Dictionary<CommentViewModel, AnimationSet>();
            ...
            try
            {
                ...
                if (Visibility == Visibility.Visible)
                {
                    if (Comments != null)
                    {
                        ...
                        await AddCommentToCanvasAsyncSafe(Comments.Cast<CommentViewModel>().ToArray());
                    }
                }
        ...
        private void AddCommentToCanvas(CommentViewModel comment, in CommentRenderFrameData frame)
        {
            CommentUI MakeCommentUI(CommentViewModel comment, in CommentRenderFrameData frame)
            {
...
//                    isCanAddRenderComment_Center = false;
        ...
        private async Task AddCommentToCanvasAsyncSafe(CommentViewModel comment)
        {
            using (await _UpdateLock.LockAsync())
            {
        ...
        private async Task AddCommentToCanvasAsyncSafe(IEnumerable<CommentViewModel> comments)
        {
            using (await _UpdateLock.LockAsync())
            {
                if (Visibility ==Visibility.Collapsed) { return; }
        ...
        private async Task RemoveCommentFromCanvasAsyncSafe(CommentViewModel comment)
        {
            using (await _UpdateLock.LockAsync())
            {
                RemoveCommentFromCanvas(comment);
        ...
        private async Task RemoveCommentFromCanvasAsyncSafe(IEnumerable<CommentViewModel> comments)
        {
            using (await _UpdateLock.LockAsync())
            {
                foreach (var comment in comments)
        ...
        Dictionary<CommentViewModel, CommentUI> _commentToRenderCommentMap = new Dictionary<CommentViewModel, CommentUI>();
        private void RemoveCommentFromCanvas(CommentViewModel comment)
        {
