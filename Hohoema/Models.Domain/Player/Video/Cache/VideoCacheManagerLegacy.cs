...
namespace Hohoema.Models.Domain.Player.Video.Cache
{
    ...
    public class VideoCacheManagerLegacy : AsyncInitialize, IDisposable
    {
        ...
        private DelegateCommand<IVideoContent> _AddCacheRequestCommand;
        public DelegateCommand<IVideoContent> AddCacheRequestCommand => _AddCacheRequestCommand
            ?? (_AddCacheRequestCommand = new DelegateCommand<IVideoContent>(video => 
            {
                _ = RequestCache(video.Id);
            }));


        private DelegateCommand<IVideoContent> _DeleteCacheRequestCommand;
        public DelegateCommand<IVideoContent> DeleteCacheRequestCommand => _DeleteCacheRequestCommand
            ?? (_DeleteCacheRequestCommand = new DelegateCommand<IVideoContent>(video =>
            {
        ...
        private async Task RestoreBackgroundDownloadTask()
        {
            ...
            foreach (var operation in tasks)
            {
                try
                {
                    ...
                    var video = await NicoVideoProvider.GetNicoVideoInfo(_info.VideoId);
                    var session = (IVideoStreamingDownloadSession)await prepareResult.CreateVideoSessionAsync(_info.Quality);
                    var progress = new NicoVideoCacheProgress(operation, session, _info.VideoId, video.Title, _info.Quality, operation.ResultFile.DateCreated.DateTime);
        ...
        private async Task TryNextCacheRequestedVideoDownload()
        {
            ...
            var videoStreamingSession = downloadSession as IVideoStreamingDownloadSession;
            ...
            try
            {
                ...
                var video = await NicoVideoProvider.GetNicoVideoInfo(nextRequest.VideoId);
                var progress = new NicoVideoCacheProgress(operation, videoStreamingSession, nextRequest.VideoId, video.Title, videoStreamingSession.Quality, nextRequest.RequestAt);
                AddDownloadOperation(progress);
        ...
        private ToastNotification MakeSuccessToastNotification(NicoVideo info)
        {
            // トーストのレイアウトを作成
        ...
        private ToastNotification MakeFailureToastNotification(NicoVideo info)
        {
            // トーストのレイアウトを作成
            ToastContent content = new ToastContent()
            {
