...
//            Debug.WriteLine("Focus on Menu : " + _isFocusMenu.Value);
    ...
    public bool IsNallowAndOpenPane(Microsoft.UI.Xaml.Controls.NavigationViewPaneDisplayMode paneDisplayMode, bool isPaneOpen)
    {
        return paneDisplayMode == Microsoft.UI.Xaml.Controls.NavigationViewPaneDisplayMode.Top
            && isPaneOpen;
    }

    public bool IsEqualsApplicationLayoutAndPlayerDisplayMode(
        ApplicationLayout appLayout, 
        ApplicationLayout targetLayout, 
        PlayerDisplayMode playerDisplayMode, 
        PlayerDisplayMode targetPlayerDisplayMode)
    {
        return appLayout == targetLayout
            && playerDisplayMode == targetPlayerDisplayMode;
    }
    public bool IsTVModeAndPlayerDisplayWithPinP(ApplicationLayout appLayout, PlayerDisplayMode playerDisplayMode)
    {
        return appLayout == ApplicationLayout.TV
            && playerDisplayMode == PlayerDisplayMode.WindowInWindow;
    }
}
...
public sealed partial class MenuItemTemplateSelector : DataTemplateSelector
{
    public DataTemplate? SeparatorTemplate { get; set; }
    public DataTemplate? LiveContentTemplate { get; set; }
    public DataTemplate? LogginUserLiveSummaryTemplate { get; set; }
    public DataTemplate? PinItemTemplate { get; set; }
    public DataTemplate? PinFolderItemTemplate { get; set; }
    public DataTemplate? MenuItemTemplate { get; set; }
    public DataTemplate? QueueItemTemplate { get; set; }
    public DataTemplate? MylistSubItemTemplate { get; set; }
    public DataTemplate? LocalMylistSubItemTemplate { get; set; }
    public DataTemplate? PinsMenuSubItemTemplate { get; set; }
    public DataTemplate? MenuSubItemTemplate { get; set; }
    public DataTemplate? SubscriptionMenuItemTemplate { get; set; }
    public DataTemplate? SubscriptionGroupMenuItemTemplate { get; set; }
    ...
    protected override DataTemplate SelectTemplateCore(object item, DependencyObject container)
    {
        ...
        {
            return SeparatorTemplate ?? throw new InvalidOperationException();
        }
        else if (item is LiveContentMenuItemViewModel)
        {
            return LiveContentTemplate ?? throw new InvalidOperationException();
        }
        else if (item is LogginUserLiveSummaryItemViewModel)
        {
            return LogginUserLiveSummaryTemplate ?? throw new InvalidOperationException();
        }
        else if (item is PinMenuItemViewModel)
        {
            return PinItemTemplate ?? throw new InvalidOperationException();
        }
        else if (item is PinFolderMenuItemViewModel)
        {
            return PinFolderItemTemplate ?? throw new InvalidOperationException();
        }
        else if (item is MenuItemViewModel)
        {
            return MenuItemTemplate ?? throw new InvalidOperationException();
        }
        else if (item is QueueMenuItemViewModel)
        {
            return QueueItemTemplate ?? throw new InvalidOperationException();
        }
        else if (item is MylistSubMenuMenu)
        {
            return MylistSubItemTemplate ?? throw new InvalidOperationException();
        }
        else if (item is LocalMylistSubMenuItemViewModel)
        {
            return LocalMylistSubItemTemplate ?? throw new InvalidOperationException();
        }
        else if (item is PinsMenuSubItemViewModel)
        {
            return PinsMenuSubItemTemplate ?? throw new InvalidOperationException();
        }
        else if (item is MenuSubItemViewModelBase)
        {
            return MenuSubItemTemplate ?? throw new InvalidOperationException();
        }
        else if (item is SubscriptionMenuItemViewModel)
        {
            return SubscriptionMenuItemTemplate ?? throw new InvalidOperationException();
        }
        else if (item is SubscriptionGroupMenuItemViewModel)
        {
            return SubscriptionGroupMenuItemTemplate ?? throw new InvalidOperationException();
        }
