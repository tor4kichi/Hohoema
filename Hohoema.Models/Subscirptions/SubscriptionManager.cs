...
namespace Hohoema.Models.Subscriptions
{
    ...
    public sealed class SubscriptionManager
    {
        ...
        static private async Task<List<NicoVideo>> GetUserVideosFeedResult(string userId, UserProvider userProvider)
        {
            ...
            if (videoItems != null || (videoItems?.Count ?? 0) > 0)
            {
                return videoItems;
            }
            else
        ...
        static private async Task<List<NicoVideo>> GetChannelVideosFeedResult(string channelId, ChannelProvider channelProvider, NicoVideoProvider nicoVideoProvider)
        {
        ...
        static private async Task<List<NicoVideo>> GetKeywordSearchFeedResult(string keyword, SearchProvider searchProvider)
        {
            int page = 0;
            const int itemGetCountPerPage = 50;
            ...
            if (res.VideoItems != null && res.ItemsCount > 0)
            {
                return res.VideoItems.ToList();
            }
            else
            {
                return new List<NicoVideo>();
        ...
        static private async Task<List<NicoVideo>> GetTagSearchFeedResult(string tag, SearchProvider searchProvider)
        {
            int page = 0;
            const int itemGetCountPerPage = 50;
