...
namespace Hohoema.Models.Repository.Niconico.Mylist
{
    ...
    public sealed class LoginUserMylistProvider : ProviderBase
    {
        ...
        public async Task<List<LoginUserMylistPlaylist>> GetLoginUserMylistGroups()
        {
            ...
            foreach (var mylistGroup in res ?? Enumerable.Empty<LoginUserMylistGroup>())
            {
                var mylist = new LoginUserMylistPlaylist(mylistGroup.Id, this)
                {
                    ...
                    IconType = mylistGroup.GetIconType().ToModelIconType(),
                    DefaultSort = mylistGroup.GetDefaultSort().ToModelDefaultSort(),
        ...
        public async Task<ContentManageResult> AddMylist(string name, string description, bool is_public, MylistGroupDefaultSort default_sort, MylistGroupIconType iconType)
        {
            var result =  await ContextActionAsync(async context =>
            {
                return await context.User.CreateMylistGroupAsync(name, description, is_public, default_sort.ToInfrastructureDefaultSort(), iconType.ToInfrastructureIconType());
            });
        ...
        public async Task<ContentManageResult> UpdateMylist(string myylistId, MylistGroupEditData editData)
        {
            if (myylistId == "0")
            {
                throw new Exception();
            ...
            var result = await ContextActionAsync(async context =>
            {
                return await context.User.UpdateMylistGroupAsync(
                    ...
                    editData.IsPublic,
                    editData.MylistDefaultSort.ToInfrastructureDefaultSort(),
                    editData.IconType.ToInfrastructureIconType()
                    );
            });
