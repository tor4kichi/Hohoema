...
namespace Hohoema.Database.Local.Subscription
{
    ...
    public static class SubscriptionDb
    {
        ...
        {
            var db = HohoemaLiteDb.GetLocalLiteRepository();
            return db.DeleteMany<SubscriptionData>(x => x.Id == subscription.Id) > 0;
        }
    }
    ...
    public static class SubscriptionFeedResultDb
    {
        ...
        {
            var db = HohoemaLiteDb.GetTempLiteRepository();
            return db.DeleteMany<SubscriptionFeedResult>(x => x.SubscriptionId == subscription.Id) > 0;
        }
    }
}
