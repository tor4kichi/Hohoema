...
namespace NicoPlayerHohoema.Models
{
	...
	public class NiconicoContentProvider 
	{
        ...
        {
            Mntone.Nico2.Live.Video.NicoliveVideoInfoResponse response = null;
            using (var releaser = await _NicoPageAccessLock.LockAsync())
            {
                await WaitNicoPageAccess();

                response = await Context.Live.GetLiveVideoInfoAsync(liveId);
            }
        ...
        {
            Mntone.Nico2.Live.Watch.ProgramInfo programInfo = null;
            using (var releaser = await _NicoPageAccessLock.LockAsync())
            {
                await WaitNicoPageAccess();

                programInfo = await Context.Live.GetProgramInfoAsync(liveId);
            }
