...
using Hohoema.Models.Live.Niwavided;
...
namespace Hohoema.UseCase.NicoLive
{
    ...
    {
        public LiveChatData Comment { get; set; }
    ...
    public class NicoLiveVideo : FixPrism.BindableBase, IDisposable
    {
            ...
            )
        {
            ...
            _LiveComments = new ObservableRangeCollection<LiveChatData>();
        ...
        TimeshiftProgram _TimeshiftProgram;
        ...
        public bool IsWatchWithTimeshift
        {
            get
            {
                ...
                return reservtionStatus != null &&
                    (reservtionStatus == ReservationStatus.FIRST_WATCH ||
                    reservtionStatus == ReservationStatus.WATCH ||
                    reservtionStatus == ReservationStatus.PRODUCT_ARCHIVE_WATCH ||
                    reservtionStatus == ReservationStatus.TSARCHIVE
                    );
            }
        }
        ...
        private ObservableRangeCollection<LiveChatData> _LiveComments;
        ...
        private LiveStatusType _LiveStatus;
        ...
        public DateTimeOffset OpenTime => _PlayerProps.OpenTime;
        public DateTimeOffset StartTime => _PlayerProps.StartTime;
        public DateTimeOffset EndTime => _PlayerProps.EndTime;


        LivePlayerProps _PlayerProps;
        ...
        private readonly LiveCommentFilteringRepository _liveCommentFilteringRepository;
        ...
        public async Task UpdateLiveStatus(LivePlayerProps props)
        {
            LiveStatus = props.LiveStatus;

            if (LiveStatus != LiveStatusType.OnAir || LiveStatus != LiveStatusType.ComingSoon)
            {
            ...
            if (props != null)
            {
                ...
                _CommunityId = props.SocialGroupId;

                LiveTitle = props.Title;
                RaisePropertyChanged(nameof(LiveTitle));
                BroadcasterId = props.BroadcasterId;
                BroadcasterName = props.BroadcasterName;
                BroadcasterCommunityType = props.BroadcasterCommunityType;
                BroadcasterCommunityId = props.BroadcasterCommunityId;
            }
        }
        ...
        public async Task StartLiveWatchingSessionAsync()
        {
            try
            {
                _PlayerProps = await NicoLiveProvider.GetPlayerPropsAsync(LiveId);
            ...
            if (_PlayerProps != null && 
                (LiveStatus == LiveStatusType.OnAir || LiveStatus == LiveStatusType.ComingSoon)
                )
            {
                Debug.WriteLine(_PlayerProps.BroadcasterId);
                ...
                if (Live2WebSocket == null)
                {
                    ...
                    var quality = _playerSettingsRepository.DefaultLiveQuality;

                    _IsLowLatency = _playerSettingsRepository.LiveWatchWithLowLatency;
                ...
                (LiveStatus == LiveStatusType.Closed)
                )
            {
                // タイムシフト視聴
                ...
                {
                    var dialog = _messageDialogService;
                    ...
                    .Subscribe(_ =>
                    {
                        _UIScheduler.Schedule(async () =>
                        {
                            ...
                            using (await _CommentRecievingLock.LockAsync())
                            {
                                ...
                                var commentDisplayDurationVpos = (int)(_playerSettingsRepository.CommentDisplayDuration.TotalMilliseconds * 0.1);
                                var commentHiddenPositionVpos = positionFromOpenVpos - (commentDisplayDurationVpos + 500);
                                var removeComments = DisplayingComments.Where(x => x.Vpos < commentHiddenPositionVpos).ToArray();
        ...
        {
            if (_PlayerProps?.Description == null) { return null; }

            var desc = _PlayerProps.Description;

            ApplicationTheme appTheme;
            if (_appearanceSettingsRepository.Theme == ElementTheme.Dark)
            {
                appTheme = ApplicationTheme.Dark;
            }
            else if (_appearanceSettingsRepository.Theme == ElementTheme.Light)
            {
                appTheme = ApplicationTheme.Light;
            }
            else
            {
                appTheme = SystemThemeHelper.GetSystemTheme();
            }

            return await HtmlFileHelper.PartHtmlOutputToCompletlyHtml(LiveId, desc, appTheme);
        }
        ...
        private async Task RefreshLeoPlayer()
        {
            ...
            _UIScheduler.Schedule(async () =>
            {
                try
                {
                    ...
                    var amsCreateResult = await NiconicoSession.CreateAdaptiveMediaSource(new Uri(_HLSUri));
        ...
        private async void Live2WebSocket_RecieveCurrentRoom(Live2CurrentRoomEventArgs e)
        {
            ...
            if (e.MessageServerType == "niwavided")
            {
                if (IsWatchWithTimeshift)
                {
                    ...
                    _NicoLiveCommentClient = new NiwavidedNicoTimeshiftCommentClient(
                        e.MessageServerUrl,
                        e.ThreadId,
                ...
                {
                    _NicoLiveCommentClient = new NiwavidedNicoLiveCommentClient(
                        e.MessageServerUrl,
                        e.ThreadId,
                        NiconicoSession.UserIdString
                        );
                ...
                if (!(_NicoLiveCommentClient is NiwavidedNicoTimeshiftCommentClient))
                {
                    _NicoLiveCommentClient.Open();

                    await Task.Delay(DeviceTypeHelper.IsMobile ? 3000 : 500);
                }
            }
        }
        ...
        public bool CanPostComment => (LiveStatus == LiveStatusType.OnAir || LiveStatus == LiveStatusType.ComingSoon);
        ...
        {
            if (_NicoLiveCommentClient is Models.Live.Niwavided.NiwavidedNicoLiveCommentClient)
            {
                var client = _NicoLiveCommentClient as NiwavidedNicoLiveCommentClient;
        ...
        public ObservableRangeCollection<LiveChatData> DisplayingComments { get; } = new ObservableRangeCollection<LiveChatData>();
        ...
        {
            var range = (int)_playerSettingsRepository.CommentDisplayDuration.TotalSeconds + PreviousProcessingRangeCommentsTimeInSeconds;
            ...
            var addComments = candidateChatItems.Select(x => x).ToArray();
            if (addComments.Any())
            {
        ...
        LiveCommentViewModel ChatToComment(LiveChatData x)
        {
            var comment = new LiveCommentViewModel();
        ...
        private async void _NicoLiveCommentClient_CommentRecieved(object sender, CommentRecievedEventArgs e)
        {
            ...
            else
            {
                ...
                if (LiveElapsedTime - TimeSpan.FromMilliseconds(e.Chat.Vpos * 100) < _playerSettingsRepository.CommentDisplayDuration)
                {
                    if (!_liveCommentFilteringRepository.IsNgCommentOwner(e.Chat.UserId) && !(e.Chat.IsOperater && e.Chat.HasOperatorCommand))
                    {
                        using (await _CommentRecievingLock.LockAsync())
                        {
                            DisplayingComments.Add(e.Chat);
                        }
                    }
                }
            }
